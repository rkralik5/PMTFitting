...
/// This is different Gauss than in the nominal PMTFit script!
Double_t Gaus(double x, double q0, double q, double s0, double s, double n){
	double sigma_n = n*q*s*s + s0*s0;
	return exp(-pow(x-q0-n*q,2)/(2*sigma_n))/sqrt(2*PI*sigma_n);
}
...
/// @brief Events that missed the first dynode
Double_t PMTFMiss(Double_t *x_, Double_t *par);
...
void PMTFitThesis(std::string inFileName, std::string outFileName="Output.csv"){
  ...
  Int_t nfound = s->Search(hCharge,5,"same",0.005);
	...
	pmt->SetNpx(1000);
  pmt->SetParNames("Q_{0}","#sigma_{0}","Q_{1}","#sigma_{1}", "Missed fraction",
									 "1^{st} dynode ampl.", "True #PE", "Scaling factor");
	...
	pmt->SetParameter(4,0.01); // Fraction of events that missed the first dynode
	pmt->SetParameter(5,3); // Multiplication at the first dynode (fraction of q1)
	...
	pmt->SetParLimits(4,1e-7,1);
	pmt->SetParLimits(5,1,10);
	...
	TF1 *pmtMiss = new TF1("pmt2",PMTFMiss,min,max,8);
	FixFit(pmt0, pmt);
	FixFit(pmt1, pmt);
	FixFit(pmt2, pmt);
	FixFit(pmtMiss, pmt);
	...
	pmt2->SetLineStyle(2);
	pmtMiss->SetLineColor(kGreen+1);
	pmtMiss->SetLineStyle(2);
	...
	leg.AddEntry(pmtMiss,"Missed 1st dynode","l");
	leg.Draw("same");
	...
	c.SetLogy();
	c.SaveAs((plotFileName+"_Logy"+"_Gate"+std::to_string(fGate)+
						"_ThesisFit.pdf").c_str());
...
Double_t PMTF(Double_t *x_, Double_t *par){
	...
	return scale*Pois(mu, 0)*Gaus(x,q0,0,s0,0,0) +
				 scale*(1-Pois(mu,0))*w*Gaus(x,q0,q1/a,s0,s1,1) +
				 scale*Pois(mu, 1)*(1-w)*Gaus(x,q0,q1,s0,s1,1) + 
				 scale*Pois(mu, 2)*(1-w)*Gaus(x,q0,q1,s0,s1,2) +
		     scale*Pois(mu, 3)*(1-w)*Gaus(x,q0,q1,s0,s1,3) + 
		     scale*Pois(mu, 4)*(1-w)*Gaus(x,q0,q1,s0,s1,4);
}
...
Double_t PMTF1(Double_t *x_, Double_t *par){
	...
	return scale*Pois(mu,1)*(1-w)*Gaus(x,q0,q1,s0,s1,1);
}
...
Double_t PMTF2(Double_t *x_, Double_t *par){
	...
	return scale*Pois(mu,2)*(1-w)*Gaus(x,q0,q1,s0,s1,2);
}
...
Double_t PMTFMiss(Double_t *x_, Double_t *par){
	...
	return scale*(1-Pois(mu,0))*w*Gaus(x,q0,q1/a,s0,s1,1);
}
