...
void PMTFit(std::string inFileName, std::string outFileName="Output.csv"){
	...
	Float_t Baseline = 0; ///< Baseline voltage in mV
	Float_t MinVoltage = 0; ///< Minimum voltage in mV
	Float_t MinTime = 0; ///< Time of minimum in ns
	tWaves->SetBranchAddress("Charge", &Charge);
	tWaves->SetBranchAddress("Baseline", &Baseline);
	tWaves->SetBranchAddress("MinVoltage", &MinVoltage);
	tWaves->SetBranchAddress("MinTime", &MinTime);
	...
	for (int i = 0; i < NEntries; i++){ // fill histogram
		...
		if(MinVoltage < fThreshold || MinTime < fPeakTime-20 || MinTime > fPeakTime+20)
  ...
  pmt->SetParNames("Q_{0}","#sigma_{0}","Q_{1}","#sigma_{1}", "w", "a", "#mu",
	...
	c.SetLogy();
	c.SaveAs((plotName+"_Logy"+".pdf").c_str());
	...
	if(CheckEmptyFile(outFileName)){
		outfile << "path,description,channel,pmt,model,voltage,chisqr,Q1,sigma,pe_res,gain_fit,gain_spe,gain_npe,gain_ped,pv_r_hist,n_pe,baseline\n";
	}
